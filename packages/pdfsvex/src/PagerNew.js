...
export class PagerNew {
	...
	walk(appendToNode, nodeToCopy, root = false) {
		...
		if (nodeToCopy.nodeType !== nodeToCopy.TEXT_NODE) {
			...
			avoidBreakInside = computedStyles.getPropertyValue('page-break-inside') === 'avoid';
			pageBreakAfter = computedStyles.getPropertyValue('page-break-after') === 'always';
		}
		...
		if (hasOverflow || pageBreakAfter || (hasOverflow && avoidBreakInside)) {
			...
			if (nodeToCopy.nodeType === nodeToCopy.TEXT_NODE) {
				...
				const parentNodeCurrentPage = this.createNode(nodeToCopy.parentElement);
				...
				for (let i = 0; i < words.length; i++) {
					...
					if (this.currentPage.hasOverflow()) {
						const lastWordIndex = textNodeCurrentPage.textContent.lastIndexOf(word);
						textNodeCurrentPage.textContent = textNodeCurrentPage.textContent.substring(
							0,
							lastWordIndex
						);
						break;
				...
				node.textContent = words.slice(lastSuccessfullWord + 1, words.length).join(' ');
			}
	...
	createNode(nodeToCopy) {
		...
		if (nodeToCopy.nodeType === nodeToCopy.TEXT_NODE) {
			...
			if (nodeToCopy.attributes) {
				const attributes = Array.prototype.slice.call(nodeToCopy.attributes);
				let attr;
